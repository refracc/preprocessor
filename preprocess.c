...
int main(int argc, char const *argv[]) // Main method generated by atom.io
{
	...
	printf("Please enter the name of the file you wish to scan in: ");
	char param[80];
	scanf("%s\n", param);
	char* outputExtension = ".o";
	FILE* input = fopen(param, "r"); // Input file for read access only.
	char line [LINE_LENGTH];
	int nonEmptyLines = 0;
	int commentedLines = 0;

	FILE* output = fopen(strcat(param, outputExtension), "w"); // Output file for write access // generates if it doesn't exist.
